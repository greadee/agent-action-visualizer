...
The focus payload contains only session and activity metadata: current and previous node IDs/paths, secondary node IDs/paths, operation, source, confidence, and timestamp. Current focus takes visual precedence over previous focus, which takes precedence over secondary focus when roles overlap.
