...
The frontend subscribes to both events, then calls `Resync()` for an authoritative graph and focus view. This subscribe-before-snapshot order closes the reconnect race: an event arriving first wins for focus, while graph revisions reject older state. Patches at or below the current revision are ignored. Newer patches update nodes by stable ID and replace the structural edge set, so additions, removals, and moves cannot leave stale edges. A frontend reload or bridge reconnect therefore converges on a deterministic full snapshot without pausing collection.
...
The focus payload contains only session and activity metadata: current and previous node IDs/paths, secondary node IDs/paths, operation, source, confidence, timestamp, and the ordered access trail. Trail entries carry sequence/timing/operation/source metadata plus optional calculated line counts and work evidence state; they never carry source contents. Work results may emit a later `aav:focus` update after the asynchronous diff worker completes. Current focus takes visual precedence over previous focus, which takes precedence over secondary and older-trail focus when roles overlap.
